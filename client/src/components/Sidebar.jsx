...
const Sidebar = () => {
    ...
    const sidebarContent = (
        <>
            ...
            <div className="p-3 border-t border-white/6">
                <button onClick={handleLogout} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/8 translate-all duration-150">
    ...
    return (
        <>
            ...
            {/* sidebar mobile */}
            <aside className={`lg:hidden fixed inset-y-0 left-0 z-50 w-72 bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 text-white flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
                {sidebarContent}
            </aside>
        </>
    )
