...
const AttendanceStats = ({ history }) => {
    ...
    const stats = [
        ...
        {label : "Days Late", value: totalLate, icon: AlertCircleIcon},
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8">
          {stats.map((s) => (
              <div key={s.label} className="card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group">
                  <div className="absolute left-0 top-0 bottom-0 w-1 rounded-r-full bg-slate-500/50 group-hover:bg-indigo-500/50" />
                  ...
                  <div>
                      ...
                      <p className="text-xl font-medium text-slate-900 tracking-tight">{s.value}</p>
                  </div>
              </div>
          ))}
    </div>
