...
const Employees = () => {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
          {filtered.length === 0 ? (
            ...
            filtered.map((emp) => (
              <EmployeeCard key={emp.id} employee={emp} onDelete={fetchEmployees} onEdit={(e) => setEditEmp(e)} />
            ))
          )}
        </div>
      )}
      ...
      {showCreateModel && (
        <div className="fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center overflow-y-auto" onClick={() => setShowCreateModel(false)}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 pb-0">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">Creare a user account and employee profile</p>
              </div>
            ...
            <div className="p-6">
              <EmployeeForm onSuccess={() => setShowCreateModel(false)} onCancel={() => setShowCreateModel(false)} />
            </div>
          </div>
        </div>
      )}
      ...
      {/* Edit Employee Model */}
      {editEmp && (
        <div className="fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center overflow-y-auto" onClick={() => setEditEmp(null)}>
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 pb-0">
              <div>
            ...
            <div className="p-6">
              <EmployeeForm initialData={editEmp} onSuccess={() => setEditEmp(null)} onCancel={() => setEditEmp(null)} />
            </div>
          </div>
        </div>
      )}
